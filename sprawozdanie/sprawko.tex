...
%\begin{figure}[!htpb]
%\begin{center}
%TODO Uzupełnić plots.gp o poniższy wykres
...
Jeżeli priorytetem jest krótki czas pracy i~dobra ocena czasu potrzebnego na znalezienie rozwiązania przed rozpoczęciem pracy, to~algorytm heurystyczny jest najlepszym wyborem.

Algorytm losowy pracował w~czasie podobnym do~czasu pracy algorytmów Steepest descent oraz Greedy, ale~otrzymywał zawsze gorsze rezultaty.
Wyjątkami od~tej reguły są zadania o~wielkościach 14, 16~i~26, jednak jego dwaj konkurenci na~tych zadaniach osiągają prawie zawsze optymalne rozwiązania.
Podsumowując, uważamy, że~algorytm losowy może służyć baza do~porównania jakości działania bardziej rozbudowanych algorytmów, bowiem dobrze obrazuje rodzaj przestrzeni rozwiązań.
Jeżeli znalezienie rozwiązania jest proste, to~jego wartość średnia jest wyższa niż dla~bardziej skomplikowanych krajobrazów funkcji kosztu.

Porównanie algorytmów Greedy i~Steepest descent jest najtrudniejsze.
Zauważyliśmy, że~Greedy wykonuje więcej kroków, ale~czas pracy jest zawsze krótszy.
Nie udało nam się znaleźć wśród testowanych instancji problemów, dla których ta cecha byłaby istotną zaletą.
Osiągane przez niego wyniki są zawsze równe lub~nieznacznie gorsze od~ Steepest descent.
Z~uwagi na~złożoność obliczeniową algorytm Steepest descent zastosowalibyśmy do~mniejszych problemów, podczas gdy do~większych bardziej odpowiedni byłby Greedy.
\end{document}
